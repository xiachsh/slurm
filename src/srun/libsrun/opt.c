...
static void _set_options(const int argc, char **argv)
{
	...
	while ((opt_char = getopt_long(argc, argv, opt_string,
		...
		case LONG_OPT_TASK_EPILOG:
			...
			break;
		case LONG_OPT_NICE:
			if (_check_jobpack__opt("--nice")) {
				if (optarg)
					opt.nice = strtol(optarg, NULL, 10);
				else
					opt.nice = 100;
				if (abs(opt.nice) > NICE_OFFSET) {
					error("Invalid nice value, must be "
					      "between -%d and %d", NICE_OFFSET,
					      NICE_OFFSET);
					exit(error_exit);
				}
				if (opt.nice < 0) {
...
"            [--acctg-freq=<datatype>=<interval>] [--pack-group]\n"
